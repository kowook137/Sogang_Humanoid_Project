...
- Jetson 및 ROS 통합은 PC 환경에서 알고리즘을 검증한 뒤 진행 예정

현재 단계는 규칙 기반 기준선까지 구현한 초기 프로토타입입니다. 개발 영상 20개에 맞춘 결과이므로 일반화 성능은 아직 검증되지 않았으며, Linux/Jetson 실시간 운용도 별도 검증이 필요합니다.
