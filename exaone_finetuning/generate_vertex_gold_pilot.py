...
의미와 안전 정보를 유지한 현대 부산·경남 존댓말 후보 세 개를 작성하라. 실제 부산의
젊은 성인이 어르신에게 편안하게 말하는 정도로 쓰고, 모든 문장을 사투리로 꾸미지 마라.
...
- '안녕하십니꺼', '참말로', '쪼매쪼매', '아입니꺼' 같은 연출된 표현은 쓰지 않는다.
- 한 문단에서 눈에 띄는 방언 어미는 두세 번 이내로 제한하고 나머지는 자연스러운
  한국어 존댓말을 사용한다.
- 답변은 안전상 상세 설명이 필요한 경우가 아니면 1~4문장으로 간결하게 쓴다.
