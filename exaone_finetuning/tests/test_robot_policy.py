...
class RobotPolicyTest(unittest.TestCase):
    def test_fall_response_never_recommends_immediate_lift(self):
        ...
        self.assertIn("바로 일으키지 마세요", result.response)
    ...
    def test_temperature_requires_value_and_timestamp(self):
        ...
        self.assertEqual(measured.response, "오후 3시에 측정된 값은 24도입니다.")
    ...
    def test_name_schedule_and_corrected_destination(self):
        ...
        self.assertEqual(destination.response, "현재 목적지는 서면역입니다.")

    def test_policy_outputs_do_not_hardcode_rejected_dialect(self):
        policy = RobotPolicy()
        responses = [
            policy.process("지금 방 안이 몇 도예요?").response,
            policy.process("오늘 우리 동네 날씨가 어때요?").response,
            policy.process("어르신이 넘어졌어요.").response,
        ]
        for response in responses:
            self.assertNotIn("니더", response)
            self.assertNotIn("심더", response)
